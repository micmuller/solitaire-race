...
async function connect(matchId, role, { route=true, reconnect=false }={}) {
  ...
  status('Verbindung wird aufgebaut …'); await client.connect({reconnect}); $('#connection-dot').classList.add('online'); overlayOpen($('#start-overlay'),false); if(route)setRoute(matchId,role); updateMeta();
...
function returnToLobby(){ const matchId=client?.matchId;if(client&&!client.matchId.startsWith('demo'))stopBots(baseUrl,client.matchId).catch(()=>{});client?.close?.();client=null;activeGame=null;activeKind='human';selection=null;resetGameOver();board.clearTransient();if(matchId)sessionStorage.removeItem(matchSessionKey(matchId));clearRoute();overlayOpen($('#menu-overlay'),false);overlayOpen($('#start-overlay'),true);$('#connection-dot').classList.remove('online');refreshLobby().catch(()=>{}); }
...
$('#nickname').value=storageGet(STORAGE.nickname)||'HighNoon'; $('#menu-nickname').value=$('#nickname').value; $('#quality').value=quality.name; $('#mute').checked=storageGet(STORAGE.mute)==='1'; $('#server-url').value=baseUrl; $('#menu-seed').value=generateRandomSeed(); all('[data-stock-side]').forEach((button)=>button.setAttribute('aria-pressed',String(button.dataset.stockSide===stockSide)));
