...
export const WEB_PIXI_CLIENT_VERSION = '0.3.3';
...
function canSendActions({ automated=false }={}) { return !isVisualPreview()&&(automated||!visualBotController?.running)&&interactionAllowed({ locked:inputLock.locked, current:client?.current, role:client?.clientId, lobbyStatus:activeGame?.status }); }
