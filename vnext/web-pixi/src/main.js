...
export const WEB_PIXI_CLIENT_VERSION = '0.1.0';
...
const board = new BoardScene(pixi, {
  quality,
  canInteract: () => !inputLock.locked && Boolean(client?.current) && client.clientId !== 'observer' && client.current.state.status === 'active' && activeGame?.status !== 'waiting',
...
async function connect(matchId, role, { route=true }={}) {
  ...
  status('Verbindung wird aufgebaut …'); await client.connect(); $('#connection-dot').classList.add('online'); overlayOpen($('#start-overlay'),false); if(route)setRoute(matchId,role); updateMeta(); status(role==='observer'?'Beobachtermodus':'Dein Tisch ist bereit','Verbunden');
}
...
async function sendIntent(kind,payload) {
  if(!client||inputLock.locked)return; inputLock.lock('pending'); $('#board-lock').hidden=false; board.setPending(true); debug(`intent ${kind} seq=${client.nextSeq}`);
...
async function refreshLobby(){ const result=await listLobbyGames(baseUrl), list=$('#lobby-games'); list.replaceChildren(); for(const game of result.games||[]){ const row=document.createElement('div'); row.className='lobby-game'; const label=document.createElement('span'); label.textContent=`${game.name} · ${game.players.p1?.nickname||'P1'} vs ${game.players.p2?.nickname||'offen'} · ${game.mode}`; const button=document.createElement('button'); button.textContent='Beitreten'; button.disabled=game.status==='finished'||Boolean(game.players.p2); button.onclick=async()=>{try{await ensurePlayer(); const joined=await joinLobbyGame(baseUrl,game.gameId,{sessionId:lobbyPlayer.sessionId}); activeGame=joined.game; await connect(joined.matchId,'p2');}catch(e){showToast(e.message,'error')}}; row.append(label,button); list.append(row); } if(!list.children.length)list.textContent='Keine offenen Spiele.'; }
...
function returnToLobby(){ if(client&&!client.matchId.startsWith('demo'))stopBots(baseUrl,client.matchId).catch(()=>{});client?.close?.();client=null;selection=null;board.clearTransient();overlayOpen($('#menu-overlay'),false);overlayOpen($('#start-overlay'),true);$('#connection-dot').classList.remove('online');refreshLobby().catch(()=>{}); }
...
const launch=readLaunchParams(location.search); if(new URLSearchParams(location.search).get('demo')==='1')startDemo(); else if(launch)connect(launch.matchId,launch.role).catch(e=>showToast(e.message,'error')); else refreshLobby().catch(()=>{});
