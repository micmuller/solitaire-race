...
import { gameForMatch, guestSessionCandidate, interactionAllowed, resignDecision, retryableSequenceReject, sameTableauSelection, seedForHostedGame, waitingMatchMessage } from './bridge/match-context.js';
...
function startDemo(){ client={clientId:'p1',matchId:'demo-4-2',current:structuredClone(DEMO_CURRENT)}; board.setLocalId('p1'); renderCurrent(client.current,'snapshot'); overlayOpen($('#menu-overlay'),false); $('#connection-dot').classList.add('online'); status('Repräsentativer autoritativer Snapshot-Fixture','Demo'); }
...
function updateMenuControls() {
  ...
  $('#menu-mode').value=mode; updateMeta();
}
...
$('#menu-open').onclick=()=>{updateMenuControls();openMenuTab(client?'game':'lobby');overlayOpen($('#menu-overlay'),true);}; $('#profile-open').onclick=()=>overlayOpen($('#profile-overlay'),true); all('[data-close]').forEach((button)=>button.onclick=()=>{const overlay=button.closest('.overlay');if(overlay.id==='menu-overlay'&&!client){openMenuTab('lobby');return;}overlayOpen(overlay,false);}); all('[data-menu-tab]').forEach((button)=>button.onclick=()=>openMenuTab(button.dataset.menuTab));
