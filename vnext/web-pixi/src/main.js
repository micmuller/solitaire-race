...
export const WEB_PIXI_CLIENT_VERSION = '0.1.3';
const $ = (selector) => document.querySelector(selector);
const all = (selector) => [...document.querySelectorAll(selector)];
const STORAGE = { nickname:'solitaire-vnext:nickname', session:'solitaire-vnext:lobbySessionId', server:'solitaire-vnext:serverBaseUrl', quality:'solitaire-pixi:quality', mute:'solitaire-pixi:mute', hud:'solitaire-pixi:hud' };
...
function updateMeta() {
  const current=client?.current, displayId=client?.clientId==='p2'?'p2':'p1';
...
$('#nickname').value=storageGet(STORAGE.nickname)||'HighNoon'; $('#menu-nickname').value=$('#nickname').value; $('#quality').value=quality.name; $('#mute').checked=storageGet(STORAGE.mute)==='1'; $('#server-url').value=baseUrl; $('#hud-toggle').checked=storageGet(STORAGE.hud)!=='0'; $('.status-meta').hidden=!$('#hud-toggle').checked; $('#menu-seed').value=generateRandomSeed();
...
$('#quality').onchange=()=>{localStorage.setItem(STORAGE.quality,$('#quality').value);showToast('Grafikqualität wird beim Neuladen aktiviert');}; $('#mute').onchange=()=>localStorage.setItem(STORAGE.mute,$('#mute').checked?'1':'0'); $('#debug-toggle').onchange=()=>$('#debug-hud').hidden=!$('#debug-toggle').checked; $('#hud-toggle').onchange=()=>{const visible=$('#hud-toggle').checked;$('.status-meta').hidden=!visible;localStorage.setItem(STORAGE.hud,visible?'1':'0');};
$('#preview-final-sequence').onclick=()=>{$('#game-over-title').textContent='Finale Vorschau';$('#game-over-text').textContent=`P1 ${client?.current?.state.players?.p1?.score??0} · P2 ${client?.current?.state.players?.p2?.score??0}`;$('#game-over').showModal();};
