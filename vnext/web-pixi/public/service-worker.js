const CACHE_NAME='solitaire-highnoon-pixi-v0.1.37';
...
self.addEventListener('activate',(event)=>{
  event.waitUntil(caches.keys().then((keys)=>Promise.all(keys.filter((key)=>key.startsWith('solitaire-highnoon-pixi-')&&key!==CACHE_NAME).map((key)=>caches.delete(key)))).then(()=>self.clients.claim()));
});
...
self.addEventListener('fetch',(event)=>{
  ...
  if(url.origin!==self.location.origin||!url.pathname.startsWith('/vnext/pixi/'))return;
